...
<div class="content">
    <b-container fluid class="h-100 w-100">
        ...
        <div class="row mt-4">
            <div v-if="dataPerson[start].status == 'l'" class="col-4 align-self-end h-100 slide-in-left" id="dicutl" style="z-index:3;">
            ...
            <div class="col text-center slide-in-bottom text-content p-5" id="text-animation">
                <p id="data" class="gettext">{{ dataPerson[start]["description"] }}</p>
                <b-button v-b-modal.modal-scrollable>อ่านเพิ่มเติม</b-button>
            </div>
            <div v-if="dataPerson[start].status == 'r'" class="col-4 align-self-end h-100 slide-in-right" id="dicutr" style="z-index:3;">
                <img :src="require('@/assets/' + dataPerson[start].person)" height="500px" />
            </div>
        </div>
        <div class="bg">
            <img :src="require('@/assets/' + dataPerson[start].bg)" class="bg-img" style="z-index:1" />
        </div>
        <div style="z-index:30;" class="navTab" id="navTab">
            <b-icon-caret-left-fill style="cursor:pointer" scale="3" @click="back" v-if="start != 0"></b-icon-caret-left-fill>
            <b-icon-house-door-fill @click="home" scale="3" style="cursor:pointer" class="mr-5 ml-5">
            </b-icon-house-door-fill>
            <b-icon-caret-right-fill style="cursor:pointer" scale="3" @click="change"></b-icon-caret-right-fill>
        </div>

        <!-- pop-up -->
        <b-modal id="modal-scrollable" style="color:black" ok-only scrollable title="Person Info">
...
.text-blur-in {
    animation: text-blur-out 0.5s cubic-bezier(0.55, 0.085, 0.68, 0.53) reverse both;
    z-index: 10;
}
